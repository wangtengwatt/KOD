...
Unsigned price and positive quantity SHALL use fixed-point strings with normalized precision at most 38, scale at most 18, and at most 38 integer digits. Exact signed price error SHALL use normalized precision at most 56 and scale at most 18 so subtraction is closed over the accepted price domain, while retaining the same 38-integer-digit cap required by `DECIMAL(56,18)`. Exponent notation, plus signs, leading zeroes, trailing decimal points, JSON numbers, negative unsigned values, zero quantities, negative zero, and any 39-integer-digit value SHALL remain invalid. The original server-provided canonical string SHALL be retained and displayed unchanged.
