...
## Seventeenth independent cross-branch review repair

A seventeenth completely fresh read-only reviewer covered client `f97c615..1a49178` and backend `f37e8cb..3683d74`; it did not inherit an earlier verdict. It reported zero Critical and four Important findings. The repairs close the remaining deleted-account GPU, RFQ, and identity lifecycle continuations and make the official rewarded-ad client support a server minimum longer than the media asset:

- GPU deposit creation/review, redemption, delist cancellation, and public product discovery now share the active-account lifecycle boundary and require an approved supplier plus a running, accepting node after waits. Account deletion rejects pending deposits, so deleted supply cannot reopen, receive a post-tombstone asset credit, or freeze a buyer's funds.
- Buyer RFQs are hidden from sellers after deletion. Quote creation locks and revalidates both buyer and supplier, while deletion closes open buyer RFQs, rejects active quotes, cancels their reserved listings, and releases the seller's exact frozen provenance.
- Identity submission acquires the account lifecycle lock before storing any document. Review reacquires the owner boundary before locking the identity row, and deletion locks and physically deletes both identity files before scrubbing ciphertext/file identifiers and closing the rows. A delayed submit or review therefore cannot resurrect PII after tombstoning.
- Reward completion now separates full-media proof from the wall-clock minimum. A short asset reports progress through its full duration, waits until `startedAt + minimumSeconds`, and then allows the normal server completion and claim flow. Rewarded-ad ISO instants retain millisecond precision in the client boundary instead of enabling a retry early through integer-second truncation.

All four principal regressions were demonstrated RED before production repair: six backend lifecycle tests failed, and the short-asset client test observed zero completion calls after the longer minimum elapsed. The repaired deletion/market/identity backend set passes 30/30 and includes RFQ freeze release, pending-deposit rejection, and physical identity-file/PII purge. The complete 11-file client feature/video set passes 131/131; the directly affected wallet API and rewarded-ad files pass 66/66.

Fresh final verification used Node `v22.23.2` and Corepack pnpm `10.33.0`. TypeScript check passes. The production build passes after transforming 5,352 main, 82 preload, and 15,029 renderer modules. Changed-file Biome checks all three client files and exits zero with only the pre-existing `RewardedVideoCard.tsx` `useConst` warning. The full client suite contains 154 files: 147 passed, 5 failed, 2 skipped; 1,536 tests: 1,476 passed, the same six classified baseline failures, and 54 skipped. Full Biome remains the exact unchanged baseline of 13 errors and 1,010 warnings across 921 files.
