...
function GuidePage() {
      ...
      <Menu.Target>
        <UnstyledButton
          className={`flex items-center gap-1 rounded-lg px-2 py-1 transition-colors ${
            canSwitchLanguage ? 'hover:bg-[var(--kod-background-tertiary)]' : 'cursor-not-allowed opacity-50'
          }`}
          disabled={!canSwitchLanguage}
        ...
        >
          <ScalableIcon icon={IconLanguage} size={16} className="text-kod-tint-secondary" />
          {!isSmallScreen && (
          ...
          <ScalableIcon
            ...
            size={14}
            className="text-kod-tint-tertiary rotate-90 flex-shrink-0"
          />
        </UnstyledButton>
      </Menu.Target>
      <Menu.Dropdown>
          ...
          <Menu.Item
            ...
            rightSection={
              lang === currentLanguage ? (
                <ScalableIcon icon={IconCheck} size={14} className="text-kod-tint-brand" />
              ) : null
            }
          >
  ...
  return (
    <Stack h="100%" gap={0} className="bg-kod-background-primary">
      {/* Header */}
      ...
      <Box px="sm" pb="md" pt="sm" className="flex-shrink-0">
        <Stack gap="xs" maw="56rem" mx="auto">
          <Stack
            className="bg-kod-background-secondary rounded-md p-3"
            ...
            <Flex align="flex-end" gap={4}>
              <Textarea
                ...
                classNames={{
                  ...
                  wrapper: 'flex-1',
                  input:
                    'block w-full outline-none border-none px-2 py-1 resize-none bg-transparent text-kod-tint-primary',
                }}
                size="sm"
                value={inputValue}
              ...
              <ActionIcon
                ...
                size={32}
                variant="filled"
                color={isLoading ? 'dark' : 'kod-brand'}
                radius="xl"
              ...
              <Menu position="top-end" shadow="md" transitionProps={{ transition: 'fade-up', duration: 200 }}>
                ...
                <Menu.Dropdown>
                  <Menu.Item
                    ...
                    rightSection={<ScalableIcon icon={IconCheck} size={14} className="text-kod-tint-brand" />}
                  >
                    KOD 新手助手
                  </Menu.Item>
                </Menu.Dropdown>
      ...
      {status === 'blocked' && (
        <Box className="fixed inset-0 bg-black/50 flex items-center justify-center z-[3000]" onClick={() => reset()}>
          <Box
            className="bg-kod-background-primary rounded-lg p-6 max-w-sm mx-4"
