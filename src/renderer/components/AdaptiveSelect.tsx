  ...
  return isSmallScreen ? (
    <Drawer.NestedRoot open={drawerOpened} onOpenChange={(open) => setDrawerOpened(open)} noBodyStyles>
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="flex flex-col h-fit fixed bottom-0 left-0 right-0 outline-none bg-kod-background-primary rounded-t-lg max-h-[80vh] overflow-hidden select-none">
          <Drawer.Handle />
          {props.label && (
            <Text c="kod-tertiary" size="xs" className="text-center my-xxs">
              {props.label}
            </Text>
          )}
