        ...
        <Flex justify="space-between">
          ...
          <Menu.Label>
            <Link to="/settings/knowledge-base">
              <IconSettings2 size={16} color="var(--kod-tint-tertiary)" />
            </Link>
          </Menu.Label>
        </Flex>
          ...
          <Menu.Item key={kb.id} onClick={() => props.onSelect?.(kb)}>
            <Flex justify="space-between" align="center" gap="xs">
              <Flex gap="xs" align="center">
                <IconFile size={14} />
                <Text c={kb.id === props.currentKnowledgeBaseId ? 'kod-brand' : ''}>{kb.name}</Text>
              </Flex>
              {kb.id === props.currentKnowledgeBaseId && <IconCheck size={14} color="var(--kod-tint-brand)" />}
            </Flex>
          </Menu.Item>
        ))}
